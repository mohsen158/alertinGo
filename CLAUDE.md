...
cmd/deployer/main.go  → deploy poller, polls GitHub for new commits (runs outside Docker)
...
## Auto-Deploy Poller
`cmd/deployer/main.go` is a standalone polling daemon that checks GitHub for new commits every N seconds (default 30). When the remote branch is ahead of the local HEAD, it runs `scripts/deploy.sh` (git pull + docker compose rebuild). Runs **outside** Docker Compose so it can restart the stack without killing itself.
